...
function makeMDUIPowerCard(powerJson, playerAdeptWay = null, className = 'mdui-card') {
  console.log('[renderers.js] makeMDUIPowerCard called', { powerJson, playerAdeptWay });

  let mduiCard = document.createElement('mdui-card');
  mduiCard.className = className;
  mduiCard.clickable = true; // Assuming you want cards to be clickable for more details later

  // 1. Power Name
  let nameSpan = document.createElement('span');
  nameSpan.innerHTML = `<strong>Name:</strong> ${powerJson.name ?? '—'}`;
  nameSpan.classList.add('card-line', 'card-title-line'); // Add a class for title line styling
  mduiCard.appendChild(nameSpan);

  // 2. Extra (if populated)
  if (powerJson.extra && powerJson.extra.trim() !== '') {
    let extraSpan = document.createElement('span');
    extraSpan.innerHTML = `<strong>Extra:</strong> ${powerJson.extra}`;
    extraSpan.classList.add('card-line');
    mduiCard.appendChild(extraSpan);
  }

  // 3. Rating
  if (powerJson.rating !== undefined && powerJson.rating !== null) {
    let ratingSpan = document.createElement('span');
    ratingSpan.innerHTML = `<strong>Rating:</strong> ${powerJson.rating}`;
    ratingSpan.classList.add('card-line');
    mduiCard.appendChild(ratingSpan);
  }

  // 4. Points Per Level / Adept Way Cost Logic
  let finalPointCost = powerJson.pointsperlevel ?? '—'; // Default to standard cost

  if (playerAdeptWay !== null && playerAdeptWay.trim() !== '') {
    // Check if adeptwayrequires exists and has a matching quality
    const hasAdeptWayRequirement = powerJson.adeptwayrequires &&
                                 powerJson.adeptwayrequires.required &&
                                 powerJson.adeptwayrequires.required.oneof &&
                                 Array.isArray(powerJson.adeptwayrequires.required.oneof.quality);

    if (hasAdeptWayRequirement) {
      const matchingWay = powerJson.adeptwayrequires.required.oneof.quality.some(
        way => way === playerAdeptWay
      );

      if (matchingWay && (powerJson.adeptway !== undefined && powerJson.adeptway !== null)) {
        finalPointCost = powerJson.adeptway;
      }
    }
  }

  let costSpan = document.createElement('span');
  costSpan.innerHTML = `<strong>Cost (per level):</strong> ${finalPointCost} Power Points`;
  costSpan.classList.add('card-line');
  mduiCard.appendChild(costSpan);


  // 5. Bonus Details (complex parsing)
  if (powerJson.bonus) {
    let bonusTitleSpan = document.createElement('span');
    bonusTitleSpan.innerHTML = `<strong>Bonus:</strong>`;
    bonusTitleSpan.classList.add('card-line', 'bonus-title'); // Add class for bonus title
    mduiCard.appendChild(bonusTitleSpan);

    let bonusDetailsList = document.createElement('ul'); // Use an unordered list for multiple bonus types
    bonusDetailsList.style.marginLeft = '1.5rem'; // Indent bonus details
    bonusDetailsList.style.listStyleType = 'disc'; // Use disc bullets

    // Disable Quality
    if (powerJson.bonus.disablequality) {
      let li = document.createElement('li');
      li.textContent = `Disables Quality (GUID): ${powerJson.bonus.disablequality}`;
      bonusDetailsList.appendChild(li);
    }

    // Initiative Pass
    if (powerJson.bonus.initiativepass) {
      let li = document.createElement('li');
      li.textContent = `Initiative Pass: +${powerJson.bonus.initiativepass}`;
      bonusDetailsList.appendChild(li);
    }

    // Specific Attribute
    if (powerJson.bonus.specificattribute && powerJson.bonus.specificattribute.name) {
      let li = document.createElement('li');
      li.textContent = `Attribute (${powerJson.bonus.specificattribute.name}): +${powerJson.bonus.specificattribute.val ?? '—'}`;
      bonusDetailsList.appendChild(li);
    }

    // Select Skill
    if (powerJson.bonus.selectskill) {
      let li = document.createElement('li');
      let skillText = `Skill: +${powerJson.bonus.selectskill.val ?? '—'}`;
      if (powerJson.bonus.selectskill.skillcategories && Array.isArray(powerJson.bonus.selectskill.skillcategories.category)) {
        skillText += ` (Categories: ${powerJson.bonus.selectskill.skillcategories.category.join(', ')})`;
      }
      li.textContent = skillText;
      bonusDetailsList.appendChild(li);
    }
    
    // If any bonus details were added, append the list
    if (bonusDetailsList.children.length > 0) {
      mduiCard.appendChild(bonusDetailsList);
    } else {
      bonusTitleSpan.remove(); // If no bonuses found, remove the "Bonus:" title as well
    }
  }

  // 6. Source and Page
  let sourcePageSpan = document.createElement('span');
  sourcePageSpan.innerHTML = `<strong>Source:</strong> ${powerJson.source ?? '—'} (pg. ${powerJson.page ?? '—'})`;
  sourcePageSpan.classList.add('card-line', 'card-source-line'); // Add a class for source line styling
  mduiCard.appendChild(sourcePageSpan);
...
export function renderAdeptPowers({ adeptPowers, sectionContent }) {
  console.log('[renderers.js] renderAdeptPowers called', adeptPowers);
  const title = document.createElement('h2');
  title.textContent = 'Adept Powers';
  sectionContent.appendChild(title);
  if (!Array.isArray(adeptPowers) || !adeptPowers.length) {
    sectionContent.appendChild(document.createTextNode('No adept powers found.'));
    return;
  }
  let playerAdeptWay = null; // Initialize to null

  // Assuming window.appState.characterData.qualities is an array of quality objects
  if (window.appState.characterData.qualities && Array.isArray(window.appState.characterData.qualities)) {
      const adeptWayQuality = window.appState.characterData.qualities.find(q =>
          q.name && q.name.endsWith("'s Way")
      );

      if (adeptWayQuality) {
          playerAdeptWay = adeptWayQuality.name;
      }
  }
  adeptPowers.forEach(power => {
    console.log(`[renderers.js] Processing adept power: ${power.name || '(Unnamed)'}`);
    const powerCard = makeMDUIPowerCard(power,playerAdeptWay);
    sectionContent.appendChild(powerCard);
  });
  return;
}
